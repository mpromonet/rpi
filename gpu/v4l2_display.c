...
#define ENCODER_FORMAT OMX_COLOR_FormatYCbYCr
...
static int video_encode_init()
{
   ...
   // create video_encode
	r = ilclient_create_component(client, &video_render, "video_render", ILCLIENT_DISABLE_ALL_PORTS | ILCLIENT_ENABLE_INPUT_BUFFERS);
	if (r != 0) 
	{
   ...
   def.nPortIndex = 90;
	...
	if (OMX_GetParameter(ILC_GET_HANDLE(video_render), OMX_IndexParamPortDefinition,&def) != OMX_ErrorNone) 
	{
		printf("%s:%d: OMX_GetParameter() for video_encode port 90failed!\n",__FUNCTION__, __LINE__);
		exit(1);
	}
   print_def(def);
   ...
   if(OMX_SetParameter(ILC_GET_HANDLE(video_render), OMX_IndexParamVideoPortFormat, &format) == OMX_ErrorNone)
	{
		printf("%s:%d: OMX_SetParameter() for video_encode port 90failed!\n",__FUNCTION__, __LINE__);
		exit(1);
	}

   printf("encode to idle...\n");
   if (ilclient_change_component_state(video_render, OMX_StateIdle) == -1) 
	{
	  printf("%s:%d: ilclient_change_component_state(video_encode, OMX_StateIdle) failed", __FUNCTION__, __LINE__);
   }

   printf("enabling port buffers for 90...\n");
   if (ilclient_enable_port_buffers(video_render, 90, NULL, NULL, NULL) != 0) {
...
static int video_encode_test(const void* buffer, size_t size)
{
   ...
   int status = 0;
	...
	OMX_BUFFERHEADERTYPE *buf = ilclient_get_input_buffer(video_render, 90, 1);
	...
	else 
	{
		...
		if (OMX_EmptyThisBuffer(ILC_GET_HANDLE(video_render), buf) !=	OMX_ErrorNone) 
		{
...
static void video_encode_close()
{
   ...
   printf("disabling port buffers for 90...\n");
   ilclient_disable_port_buffers(video_render, 90, NULL, NULL, NULL);
