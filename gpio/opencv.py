...
class FaceDetect():
	def __init__(self):
		...
		for prop in [cv.CV_CAP_PROP_FRAME_WIDTH,cv.CV_CAP_PROP_FRAME_HEIGHT,cv.CV_CAP_PROP_FRAME_COUNT,cv.CV_CAP_PROP_POS_FRAMES,cv.CV_CAP_PROP_MODE ]:
			print ( str(prop) +"="+ str(cv.GetCaptureProperty(self.capture,prop)) );
	...
	def detect(self):
		...
		# detect objects
		faces = cv.HaarDetectObjects(image=self.grayscale, cascade=self.cascade, storage=self.storage, scale_factor=1.2, min_neighbors=2, flags=cv.CV_HAAR_DO_CANNY_PRUNING)
	...
	def run(self):
		...
		# create storage
		self.storage = cv.CreateMemStorage(64)
